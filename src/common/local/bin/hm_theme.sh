...
if [[ $# -eq 0 ]]; then
    ...
    DOCKBAR_STATUS=$(cat "$HOME/.local/state/z_theme/dockbar_manual_state" 2>/dev/null || echo "0")
    DOCKBAR_TEXT="OFF"
    [[ "$DOCKBAR_STATUS" == "1" ]] && DOCKBAR_TEXT="ON"

    DESKTOP_ICONS_STATUS=$(cat "$HOME/.local/state/z_theme/desktop_icons_state" 2>/dev/null || echo "0")
