...
if [[ $# -eq 0 ]]; then
    DOCK_STATUS=$(cat "$HOME/.local/state/z_theme/dockbar_autohide_state" 2>/dev/null || echo "0")
    DOCK_TEXT="OFF"
...
case "$chosen" in
    ...
    *"Open Settings Folder"*) spawn thunar "$HOME/zspace-control" ;;
    *"Open Config Menu General Tab"*) spawn code $HOME/zspace-control/zmenu-general-custom.sh ;;
