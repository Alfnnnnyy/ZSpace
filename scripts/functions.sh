...
print_header() {
    ...
    echo -e "${C_BOLD}${C_CYAN}--- WELCOME TO ZSPACE - CONFIG INSTALLER (VOID LINUX) ---${C_RESET}"
    echo "This script will help you set up your ZSpace configuration"
...
# Deploy for zspace-archive repo (Wallpaper, icons, etc.)
deploy_assets_from_archive_repo() {
    ...
    if [[ -d "$ARCHIVE_DIR/.git" ]]; then
        ...
        if [[ -d "$ARCHIVE_DIR" ]]; then
            ...
            if ask_yes_no "===> Remove and re-clone zspace-archive?"; then
                rm -rf "$ARCHIVE_DIR"
            else
...
check_control_dir() {
    if [[ ! -d "$ZSPACE_CONTROL_DIR" ]]; then
        log_warn "zspace-control directory not found. Creating..."
        mkdir -p "$ZSPACE_CONTROL_DIR"
    fi
    ...
    local required_files=(
        ...
        "hyprland-custom.lua"
        "dockbar_pin_apps"
        "zmenu-general-custom.sh"
    )
    for file in "${required_files[@]}"; do
        if [[ ! -f "$DEST_CONTROL_DIR/$file" ]]; then
            log_warn "$file not found in zspace-control. Creating default..."
            copy_file "$ZSPACE_CONTROL_DIR/$file" "$DEST_CONTROL_DIR/$file"
        fi
    done
    ...
    # Check ZSpace Control version
    if [[ -f "$DEST_CONTROL_DIR/main_setting.sh" ]]; then
        local current_version
        local source_version
        chmod +x "$ZSPACE_CONTROL_DIR/main_setting.sh"

        source_version=$("$ZSPACE_CONTROL_DIR/main_setting.sh" | grep -oP 'ZSpace Control Settings Version: \K[0-9]+\.[0-9]+\.[0-9]+')
        current_version=$("$DEST_CONTROL_DIR/main_setting.sh" | grep -oP 'ZSpace Control Settings Version: \K[0-9]+\.[0-9]+\.[0-9]+')
        if [[ "$current_version" != "$source_version" ]]; then
            log_warn "ZSpace Control version mismatch: $current_version (current) vs $source_version (expected). Updating..."
            log_warn "If you choose update, your custom settings in main_setting.sh will be overwritten."
            if ask_yes_no "===> Do you want to update zspace-control to the latest version?"; then
                copy_file "$ZSPACE_CONTROL_DIR/main_setting.sh" "$DEST_CONTROL_DIR/main_setting.sh"
                log_ok "ZSpace Control updated to version $source_version."
            else
                log_warn "You chose not to update zspace-control. Some features may not work as expected."
            fi
        else
            log_ok "ZSpace Control version is up-to-date: $current_version"
        fi
    else
        log_warn "main_setting.sh not found in zspace-control. Creating default..."
        copy_file "$ZSPACE_CONTROL_DIR/main_setting.sh" "$DEST_CONTROL_DIR/main_setting.sh"
    fi
}
